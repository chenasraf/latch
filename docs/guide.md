...
Handlers can declare capability tags at registration time, describing what they provide rather than
who they are. The registry can then be queried by tag to find matching handlers.

### Declaring capabilities

Pass tags as the second argument to `registerHandler()`:
...
These tags are separate from per-hook tags set via `tag()` — they describe the handler itself,
not individual hooks.
