...
`RegisteredHandler` that you use to attach hooks. All hooks are automatically tagged with
`handler:{name}`.
...
`DuplicateHandlerException`. Pass the `RegisteredHandler` instance around via DI or store it as
