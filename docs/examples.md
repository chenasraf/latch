...
For cross-app communication to work in Nextcloud, all apps must share the same Latch classes.
This means Latch should be installed as a Nextcloud app (not bundled in each app's `vendor/`).
If each app bundles its own copy via `composer require`, they get separate class autoloaders
and the singleton won't be shared between them.

**Setup:**

1. Install Latch as a Nextcloud app (e.g. in `apps/latch/`)
2. Each app that uses Latch should list it as a dependency in `info.xml`:
   ```xml
   <dependencies>
       <lib>latch</lib>
   </dependencies>
   ```
3. Use `LatchBootstrap::registry()` to get the shared singleton
...
class Application extends App implements IBootstrap
{
    ...
    public function boot(IBootContext $context): void
    {
        ...
        $this->myapp = $registry->registerSource('myapp')
